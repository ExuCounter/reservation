                    ...
                    callback: function() {
                        ...
                        big_pad.slideToggle(function() {
                            back_drop.hide('fade', function() {
                                if(canvas[0].toDataURL() != _get_base64_value()){
                                    if (settings.callback)
                                        settings.callback(_get_base64_value(), 'accept');
                                }else{
                                    if (settings.callback)
                                        settings.callback(false, 'accept');
                                }
                            });
                        });
                    }
...
$(document).ready(function() {
    ...
    $('#datepick, #datepick2, #datepick3, #home-arrive, #home-departure').datepicker(options);
});
...
$('#form-box input[type=text]').on('change invalid', function() {
    ...
    if (!textfield.validity.valid) {
      textfield.setCustomValidity('אנא מלא את השדה הזה.');  
    }
});
...
function expirationDate(){
  var today = new Date(),
      ...
      addYear = $('#expiryYear'),
      html = '<option  value="">שנה</option>';

  for (var i = 0; i < 11; i++, yyyy++) {
